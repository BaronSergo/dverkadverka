        ...
        */
        ...
        i18n : {
            ...
            'ru' : {
                ...
                NEXT        : 'Далее',
                PREV        : 'Назад',
                ERROR       : 'Контент не загружен <br> Пожалуйста, попробуйте снова',
                PLAY_START  : 'Слайдшоу',
...
document.addEventListener('DOMContentLoaded', function () {
    var categoriesToggler = document.querySelector('.navigation-mobile__toggler--categories');
    var menuToggler = document.querySelector('.navigation-mobile__toggler--menu');
    var categoriesList = document.querySelector('.navigation-mobile__categories');
    var menuList = document.querySelector('.navigation-mobile__menu');
    var hasSubmenuItems = document.querySelectorAll('.navigation-mobile__categories-item--hasmenu a');

    if (categoriesToggler) {
        categoriesToggler.addEventListener('click', function (evt) {
            menuList.classList.add('navigation-mobile__menu--hidden');
            categoriesList.classList.toggle('navigation-mobile__categories--hidden');
        });
    }

    if (menuToggler) {
        menuToggler.addEventListener('click', function (evt) {
            categoriesList.classList.add('navigation-mobile__categories--hidden');
            menuList.classList.toggle('navigation-mobile__menu--hidden');
        });
    }

    for (var i = 0; i < hasSubmenuItems.length; i++) {
        hasSubmenuItems[i].addEventListener('click', function (evt) {
            evt.preventDefault();
            var submenu = this.parentNode.querySelector('.navigation-mobile__subcategories');
...
$('.banners-slider__list').owlCarousel({
    loop: true,
    nav: true,
    autoplay: true,
    autoplayTimeout: 5000,
...
document.addEventListener('DOMContentLoaded', function () {
    var modalOpeners = document.querySelectorAll('.js_modal-opener');

    for (var i = 0; i < modalOpeners.length; i++) {
        modalOpeners[i].addEventListener('click', function (evt) {
            evt.preventDefault();
            var currentModalId = evt.target.getAttribute('data-modal-opener');
            var currentModal = document.querySelector('.modal[data-modal-id="' + currentModalId + '"');
            var modalClose = currentModal.querySelector('.modal__close');
            var modalOverlay = currentModal.querySelector('.modal__overlay');

            if (currentModal && currentModal.classList.contains('modal--hidden')) {
...
(function($){
	...
	var article = $('.catalog-page .article');

	var BtnShowArticle = $('.article__more-less-toggler--on');

	var BtnHideArticle = $('.article__more-less-toggler--off');

	BtnShowArticle.on('click', function() {
		article.removeClass('turn-off').addClass('turn-on');

		var calcHeightArticleBody = 0;

		$('.article__body > *:not(.bottom-shadow)').each(function () {
		    calcHeightArticleBody += parseInt( $(this).height() ) + parseInt( $(this).css( 'margin-top' ) ) + parseInt( $(this).css( 'margin-bottom' ) );
		});

		console.log(calcHeightArticleBody, $('.article__body'));

		$('.article__body').css('max-height', calcHeightArticleBody + 'px');
	});

	BtnHideArticle.on('click', function() {
		article.removeClass('turn-on').addClass('turn-off');
		$('.article__body').css('max-height', '130px');
	});
...
document.addEventListener('DOMContentLoaded', function () {
    var tabsButton = document.querySelectorAll('.tabs__button');

    for (var i = 0; i < tabsButton.length; i++) {
        tabsButton[i].addEventListener('click', function (evt) {
            var currentTabsBlock = evt.target.parentNode.parentNode;
            var currentTabId = evt.target.getAttribute('data-tab-id');
            var currentTabContent = currentTabsBlock.querySelector('.tabs__content[data-tab-id="'+ currentTabId +'"]');
            var tabsActiveButton = currentTabsBlock.querySelector('.tabs__button--active');
            var tabsActiveContent = currentTabsBlock.querySelector('.tabs__content--active');

            tabsActiveButton.classList.remove('tabs__button--active');
            tabsActiveContent.classList.remove('tabs__content--active');

            evt.target.classList.add('tabs__button--active');
            currentTabContent.classList.add('tabs__content--active');
        })
    }
});

// Внимание, для блока необходима библиотека JQuery
document.addEventListener('DOMContentLoaded', function () {
    if (window.innerWidth <= 991) {
        $('.cart-item').wrapAll("<div class='cart__items-carousel'></div>");
    }
});
